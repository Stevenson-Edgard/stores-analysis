...
def clean_sales(sales: pd.DataFrame) -> pd.DataFrame:
    # Convert Date to datetime with dayfirst=True for DMY format
    sales['Date'] = pd.to_datetime(sales['Date'], dayfirst=True, errors='coerce')
    # Fill missing values if any (example: none expected in sales)
    return sales

def clean_stores(stores: pd.DataFrame) -> pd.DataFrame:
    # No missing values expected, but can add checks
    return stores
